...
public class Lab1 {
	...
	/**
	 * Returns the largest number of Canadian quarters (each worth 25 cents)
	 * whose total value does not exceed the given number of cents.
	 * 
	 * @param cents
	 *            a positive number of cents
	 * @return the largest number of Canadian quarters (each worth 25 cents)
	 *         whose total value does not exceed the given number of cents
	 */
	...
	/**
	 * Wraps an angle given in degrees so that it lies in the range of -359
	 * degrees to 359 degrees. For example:
	 * 
	 * <pre>
	...
	*  3652         52   (wraps around to 52 degrees)
	 ...
	 * (see the lab document for the actual formula).
	 * 
	 * @param x
	 *            a value
	 ...
	 * @return true if (x, y) is inside the perimeter of the unit circle, and
	 *         false otherwise
	 */
	...
	/**
	 * Returns true if the given grade in percent is equal to a letter grade of
	 * A, and false otherwise. The minimum percent grade for a letter grade of A
	 * is 80 and the minimum percent grade for a letter grade of A+ is 90.
	 * 
	 * @param percent
	 *            a grade in percent
	 * @return true if the given grade in percent is equal to a letter grade of
	...
	public static boolean isGradeOutOfRange(double percent) {
		...
		if (percent < minGrade || percent > maxGrade)
			result = true;
		// Your implementation should use minGrade and maxGrade instead of 0 and
		// 100
		return result;
	}
	...
	* isLeapYear(2005)  returns  false  (2005 is not divisible by 4)
	 ...
	 *             if year is less than 1582 (the year the Gregorian calendar
	 *             was adopted)
	 */
	...
	public static int contains(double x, Range range) {
		...
		if (x <= range.getMinimum() || range.getMaximum() <= x) {
			return 0;
		}
		return 1;
	...
	public static int compareTo(Range r1, Range r2) {
		...
		int wid1 = (int) (r1.getMaximum() - r1.getMinimum());
		int wid2 = (int) (r2.getMaximum() - r2.getMinimum());
	 ...
	 * 
	 * <p>
	 * The returned string has the form <code>"minimum: x, maximum: y"</code>
	 * where x is the minimum value of the range and y is the maximum value of
	 * the range.
	 * 
	 * @param r
	 *            a Range instance
	...
	/**
	 * Returns the character n positions from the end of the string s. For
	 * example, suppose <code>s</code> is the string <code>"abcd"</code>; then:
	 * 
	 * <pre>
	...
	* Lab1.charFromEnd(s, -1) throws an IllegalArgumentException
	 ...
	 * @param n
	 *            the desired location of the character counting from the end of
	 *            the string
	 * @return the character located n positions from the end of the string
	 * @throws IllegalArgumentException
	 *             if the character located n positions from the end of the
	 *             string does not exist
	 */
	public static char charFromEnd(String s, int n) {

		if (s.isEmpty()) {
			throw new IllegalArgumentException("sring has length 0");
	...
	/**
	 * Sorts the elements of the list t so that the elements are in ascending
	 * order. A precondition of this method is that t must be already sorted in
	 * ascending order except that adjacent pairs of elements in t may be out of
	 * order. Consider the following almost sorted lists:
	 * 
	 * <pre>
	...
	* [0, 1, 3, 2, 4]       3, 2 is out of order
	 ...
	 * 
	 * <p>
	 * This method switches the positions of the out-of-order adjacent elements
	 * thus repairing the list so that it is in sorted order.
	 * 
	 * @param t
